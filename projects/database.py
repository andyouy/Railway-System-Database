...
class DB:
    ...
    def connect(self, database=None):
        ...
        try:
            if database: # if is not Null
                self._database = database
            connection = pymysql.connect(host=self._host, user=self._username, password=self._password, database=self._database)
            return connection
    ...
    def drop_database(self, database=None):
        ...
        :return: True if the drop is successful. Otherwise, print the error.
        """
        connection = pymysql.connect(host=self._host, user=self._username, password=self._password)
